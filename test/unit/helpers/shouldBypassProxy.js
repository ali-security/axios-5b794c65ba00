...
describe('helpers::shouldBypassProxy', function () {
  ...
  it('should bypass proxy for the whole 127.0.0.0/8 loopback range', function () {
    setNoProxy('localhost');

    assert.strictEqual(shouldBypassProxy('http://127.0.0.2:8080/'), true);
    assert.strictEqual(shouldBypassProxy('http://127.1.2.3:8080/'), true);
    assert.strictEqual(shouldBypassProxy('http://127.255.255.254:8080/'), true);

    setNoProxy('127.0.0.1');

    assert.strictEqual(shouldBypassProxy('http://127.0.0.2:8080/'), true);
    assert.strictEqual(shouldBypassProxy('http://[::ffff:127.0.0.2]:8080/'), true);

    setNoProxy('127.0.0.2');

    assert.strictEqual(shouldBypassProxy('http://127.0.0.1:8080/'), true);
    assert.strictEqual(shouldBypassProxy('http://localhost:8080/'), true);
  });

  it('should not bypass proxy for non-loopback hosts resembling the loopback range', function () {
    setNoProxy('localhost');

    assert.strictEqual(shouldBypassProxy('http://128.0.0.1:8080/'), false);
    assert.strictEqual(shouldBypassProxy('http://27.0.0.1:8080/'), false);
    assert.strictEqual(shouldBypassProxy('http://127.0.0.1.example.com:8080/'), false);
    assert.strictEqual(shouldBypassProxy('http://169.254.169.254/latest/meta-data/'), false);
  });
