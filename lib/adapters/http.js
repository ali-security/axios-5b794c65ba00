...
module.exports = function httpAdapter(config) {
  return new Promise(function dispatchHttpRequest(resolvePromise, rejectPromise) {
    ...
    // Read `auth` and its members as own properties only: a config object that
    // inherits from a polluted Object.prototype (e.g. a plain clone produced by
    // an interceptor) must never have inherited credentials silently turned into
    ...
    // Own-property read: an inherited `paramsSerializer` must not be allowed to
    // take over query-string serialization for this request.
    ...
    var options = {
      path: buildURL(parsed.path, config.params, paramsSerializer).replace(/^\?/, ''),
    ...
    // Own-property read: an inherited `proxy` must not route this request - and
    // the credentials it carries - through an attacker-controlled proxy.
