...
module.exports = function dispatchRequest(config) {
  ...
  // Flatten headers
  // Read the `common` / per-method buckets as own properties only, so a polluted
  // Object.prototype cannot inject header buckets into the outgoing request.
  var commonHeaders = utils.hasOwnProp(config.headers, 'common') && config.headers.common
    ? config.headers.common
    : {};
  var methodHeaders = config.method &&
    utils.hasOwnProp(config.headers, config.method) &&
